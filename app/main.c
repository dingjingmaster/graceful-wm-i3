...
int main(int argc, char *argv[])
{
    ...
    // 如果命令行传入更多参数，则通过IPC发给i3进程
    if (optind < argc) {
        set_verbosity(true);            // 输出更多日志
        ...
        if (ipc_send_message(sockfd, strlen(payload), I3_IPC_MESSAGE_TYPE_RUN_COMMAND, (uint8_t *)payload) == -1) {
...
#undef xmacro
    ...
    if (visual_type != NULL) {
        ...
        xcb_void_cookie_t cm_cookie = xcb_create_colormap_checked(conn, XCB_COLORMAP_ALLOC_NONE, colormap, root, visual_type->visual_id);
...
#undef xmacro
    ...
    /* Acquire the WM_Sn selection. */
    {
        ...
        atom_reply = xcb_intern_atom_reply(conn, xcb_intern_atom_unchecked(conn, 0, strlen(atom_name), atom_name), NULL);
        free(atom_name);
        if (atom_reply == NULL) {
        ...
        xcb_get_selection_owner_reply_t *selection_reply = xcb_get_selection_owner_reply(conn, xcb_get_selection_owner(conn, wm_sn), NULL);
        ...
        if (selection_reply && selection_reply->owner != XCB_NONE) {
            ...
            do {
                ...
                geom_reply = xcb_get_geometry_reply(conn, xcb_get_geometry(conn, selection_reply->owner), NULL);
