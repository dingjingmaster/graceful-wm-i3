...
Con *con_new_skeleton(Con *parent, i3Window *window)
{
    ...
    TAILQ_INSERT_TAIL(&gAllCons, new, allCons);
    new->type = CT_CON;
    new->window = window;
...
void con_free(Con *con)
{
    ...
    TAILQ_REMOVE(&gAllCons, con, allCons);
...
static void _con_attach(Con *con, Con *parent, Con *previous, bool ignore_focus)
{
    ...
    if (parent->type == CT_DOCKAREA) {
        ...
        TAILQ_FOREACH (loop, nodes_head, nodes) {
            int result = g_ascii_strcasecmp(con->window->class_class, loop->window->class_class);
            if (result == 0) {
                result = g_ascii_strcasecmp(con->window->class_instance, loop->window->class_instance);
            }
            if (result < 0) {
                current = loop;
                break;
    ...
    if (con->type == CT_FLOATING_CON) {
        DEBUG("Inserting into floating containers\n");
        TAILQ_INSERT_TAIL(&(parent->floating_head), con, floatingWindows);
    } else {
        if (!ignore_focus) {
        ...
        if (con->window != NULL &&
            ...
            DEBUG("Parent is a workspace. Applying default layout...\n");
        ...
        if (current != NULL && parent->type != CT_OUTPUT) {
            DEBUG("Inserting con = %p after con %p\n", con, current);
...
void con_detach(Con *con) {
    con_force_split_parents_redraw(con);
    if (con->type == CT_FLOATING_CON) {
        TAILQ_REMOVE(&(con->parent->floating_head), con, floatingWindows);
...
void con_focus(Con *con) {
    ...
    /* 1: set gFocused-pointer to the new con */
...
void con_activate_unblock(Con *con) {
     ...
     * warp to the currently gFocused container (!) on the target workspace.
     *
     * Therefore, before calling workspace_show(), we make sure that 'con' will
     * be gFocused on the workspace. However, we cannot just con_focus(con)
     * because then the pointer will not be warped at all (the code thinks we
     * are already there).
     *
     * So we focus 'con' to make it the currently gFocused window of the target
...
void con_close(Con *con, kill_window_t kill_window)
{
    ...
    if (con->type == CT_OUTPUT || con->type == CT_ROOT) {
        DEBUG("con = %p is of type %d, not closing anything.\n", con, con->type);
        return;
    }

    if (con->type == CT_WORKSPACE) {
        DEBUG("con = %p is a workspace, closing all children instead.\n", con);
        Con *child, *nextchild;
        for (child = TAILQ_FIRST(&(con->focus_head)); child;) {
            nextchild = TAILQ_NEXT(child, focused);
            DEBUG("killing child = %p.\n", child);
 ...
 * a tabbed / stacked parent of which they are not the currently gFocused child.
 *
 */
...
bool con_accepts_window(Con *con)
{
    ...
    if (con_is_split(con)) {
        DEBUG("container %p does not accept windows, it is a split container.\n", con);
        return false;
    }
...
Con *con_get_output(Con *con)
{
     ...
     * in the tree (root node cannot be gFocused). */
...
{
    DEBUG("Searching for parent of Con %p with orientation %d\n", con, orientation);
    Con *parent = con->parent;
    if (parent->type == CT_FLOATING_CON)
        return NULL;
    while (con_orientation(parent) != orientation) {
        DEBUG("Need to go one level further up");
        parent = parent->parent;
...
Con *con_get_fullscreen_con(Con *con, fullscreen_mode_t fullscreen_mode)
{
    ...
    while (!TAILQ_EMPTY(&bfs_head)) {
        ...
        TAILQ_FOREACH (child, &(current->floating_head), floatingWindows) {
            entry = smalloc(sizeof(struct bfs_entry));
            entry->con = child;
            TAILQ_INSERT_TAIL(&bfs_head, entry, entries);
        }
...
Con *con_get_fullscreen_covering_ws(Con *ws)
{
    ...
    Con *fs = con_get_fullscreen_con(gConnRoot, CF_GLOBAL);
    if (!fs) {
...
/*
 * Checks if the given container is inside a gFocused container.
 *
 */
...
{
    Con *con;
    TAILQ_FOREACH (con, &gAllCons, allCons) {
...
bool con_find_transient_for_window(Con *start, xcb_window_t target)
{
    ...
    int count = con_num_windows(gConnRoot);
    while (transient_con != NULL &&
           transient_con->window != NULL &&
           transient_con->window->transient_for != XCB_NONE) {
        DEBUG("transient_con = 0x%08x, transient_con->window->transient_for = 0x%08x, target = 0x%08x\n",
...
{
    g_assert(con != NULL);
    DEBUG("Toggling mark \"%s\" on con = %p.\n", mark, con);
...
{
    g_assert(con != NULL);
    DEBUG("Setting mark \"%s\" on con = %p.\n", mark, con);

    con_unmark(NULL, mark);
    if (mode == MM_REPLACE) {
        DEBUG("Removing all existing marks on con = %p.\n", con);
...
void con_unmark(Con *con, const char *name)
{
    Con *current;
    if (name == NULL) {
        DEBUG("Unmarking all containers.");
        TAILQ_FOREACH (current, &gAllCons, allCons) {
        ...
        DEBUG("Removing mark \"%s\".\n", name);
        current = (con == NULL) ? con_by_mark(name) : con;
        if (current == NULL) {
            DEBUG("No container found with this mark, so there is nothing to do.\n");
            return;
        }

        DEBUG("Found mark on con = %p. Removing it now.\n", current);
        current->mark_changed = true;
...
Con *con_for_window(Con *con, i3Window *window, Match **store_match)
{
    ...
    TAILQ_FOREACH (child, &(con->floating_head), floatingWindows) {
        TAILQ_FOREACH (match, &(child->swallow_head), matches) {
            if (!match_matches_window(match, window))
                continue;
            if (store_match != NULL)
...
int con_num_windows(Con *con)
{
    ...
    TAILQ_FOREACH (current, &(con->floating_head), floatingWindows) {
        num += con_num_windows(current);
    }
...
{
    if (con->type == CT_WORKSPACE) {
        DEBUG("You cannot make a workspace fullscreen.\n");
        return;
    }

    DEBUG("toggling fullscreen for %p / %s\n", con, con->name);
...
static void con_set_fullscreen_mode(Con *con, fullscreen_mode_t fullscreen_mode)
{
    ...
    if (con->fullscreen_mode != CF_NONE) {
        DEBUG("Setting _NET_WM_STATE_FULLSCREEN for con = %p / window = %d.\n", con, con->window->id);
        xcb_add_property_atom(gConn, con->window->id, A__NET_WM_STATE, A__NET_WM_STATE_FULLSCREEN);
    } else {
        DEBUG("Removing _NET_WM_STATE_FULLSCREEN for con = %p / window = %d.\n", con, con->window->id);
        xcb_remove_property_atom(gConn, con->window->id, A__NET_WM_STATE, A__NET_WM_STATE_FULLSCREEN);
    }
}
...
void con_enable_fullscreen(Con *con, fullscreen_mode_t fullscreen_mode)
{
    ...
    g_assert(fullscreen_mode == CF_GLOBAL || fullscreen_mode == CF_OUTPUT);

    if (fullscreen_mode == CF_GLOBAL)
        DEBUG("enabling global fullscreen for %p / %s\n", con, con->name);
    else
        DEBUG("enabling fullscreen for %p / %s\n", con, con->name);

    if (con->fullscreen_mode == fullscreen_mode) {
        DEBUG("fullscreen already enabled for %p / %s\n", con, con->name);
        return;
    }
    ...
    Con *fullscreen = con_get_fullscreen_con(gConnRoot, CF_GLOBAL);
    if (fullscreen == NULL)
    ...
    Con *cur_ws = con_get_workspace(gFocused);
    Con *old_focused = gFocused;
...
void con_disable_fullscreen(Con *con)
{
    ...
    DEBUG("disabling fullscreen for %p / %s\n", con, con->name);

    if (con->fullscreen_mode == CF_NONE) {
        DEBUG("fullscreen already disabled for %p / %s\n", con, con->name);
        return;
    }
...
static bool _con_move_to_con(Con *con, Con *target, bool behind_focused, bool fix_coordinates, bool dont_warp, bool ignore_focus, bool fix_percentage)
{
    ...
    if (!ignore_focus && !con_fullscreen_permits_focusing(target_ws)) {
        DEBUG("Cannot move out of a fullscreen container.");
        return false;
    }
    ...
    if (con->type == CT_WORKSPACE) {
        ...
        if (con == NULL) {
            ERROR("Workspace failed to move its contents into a container!");
            return false;
        }
    }
     ...
     * of this function. */
    Con *current_ws = con_get_workspace(gFocused);

    Con *source_output = con_get_output(con),
        *dest_output = con_get_output(target_ws);

    /* 1: save the container which is going to be gFocused after the current
    ...
    /* 4: re-attach the con to the parent of this gFocused container */
    Con *parent = con->parent;
    con_detach(con);
    ...
    if (!ignore_focus && !con_is_internal(target_ws) && !fullscreen) {
        /* We need to save the gFocused workspace on the output in case the
         * new workspace is hidden and it's necessary to immediately switch
         * back to the originally-gFocused workspace. */
        Con *old_focus_ws = TAILQ_FIRST(&(output_get_content(dest_output)->focusHead));
        Con *old_focus = gFocused;
        con_activate(con_descend_focused(con));

        if (old_focus_ws == current_ws && old_focus->type != CT_WORKSPACE) {
            /* Restore focus to the currently gFocused container. */
            con_activate(old_focus);
        } else if (con_get_workspace(gFocused) != old_focus_ws) {
            /* Restore focus if the output's gFocused workspace has changed. */
...
bool con_move_to_target(Con *con, Con *target) {
    ...
    if (con_get_workspace(target) == workspace_get("__i3_scratch")) {
        DEBUG("target container is in the scratchpad, moving container to scratchpad.\n");
        scratchpad_move(con);
        return true;
    }
    ...
    /* For split containers, we use the currently gFocused container within it.
     * This allows setting marks on, e.g., tabbed containers which will move
     * con to a new tab behind the gFocused tab. */
    if (con_is_split(target)) {
        DEBUG("target is a split container, descending to the currently gFocused child.\n");
...
bool con_move_to_mark(Con *con, const char *mark) {
    ...
    if (target == NULL) {
        DEBUG("found no container with mark \"%s\"\n", mark);
        return false;
    }
...
/*
 * Moves the given container to the currently gFocused container on the given
 * workspace.
 *
...
/*
 * Moves the given container to the currently gFocused container on the
...
orientation_t con_orientation(Con *con) {
    switch (con->layout) {
        ...
        case L_DOCKAREA:
        case L_OUTPUT:
            ERROR("con_orientation() called on dockarea/output (%d) container %p\n", con->layout, con);
            g_assert(false);
    }
...
/*
 * Returns the container which will be gFocused next when the given container
...
{
    /* dock clients cannot be gFocused, so we focus the workspace instead */
    if (con->parent->type == CT_DOCKAREA) {
     ...
     * it’s currently gFocused already */
    Con *next = TAILQ_FIRST(&(con->parent->focus_head));
    if (next != con) {
        DEBUG("Using first entry %p\n", next);
    } else {
...
/*
 * Returns the gFocused con inside this client, descending the tree as far as
 * possible. This comes in handy when attaching a con to a workspace at the
 * currently gFocused position, for example.
 *
 */
Con *con_descend_focused(Con *con)
{
    Con *next = con;
    while (next != gFocused && !TAILQ_EMPTY(&(next->focus_head)))
        next = TAILQ_FIRST(&(next->focus_head));
    return next;
}
...
Con *con_descend_direction(Con *con, direction_t direction)
{
    ...
    DEBUG("con_descend_direction(%p, orientation %d, direction %d)\n", con, orientation, direction);
    if (direction == D_LEFT || direction == D_RIGHT) {
        if (orientation == HORIZ) {
            ...
            /* Wrong orientation. We use the last gFocused con. Within that con,
...
static Rect con_border_style_rect_without_title(Con *con)
{
    ...
    DEBUG("The border width for con is set to: %d\n", con->current_border_width);
    Rect result;
    ...
    DEBUG("Effective border width is set to: %d\n", border_width);
...
int con_border_style(Con *con)
{
    if (con->fullscreen_mode == CF_OUTPUT || con->fullscreen_mode == CF_GLOBAL) {
        DEBUG("this one is fullscreen! overriding BS_NONE\n");
        return BS_NONE;
    }
...
{
    DEBUG("con_set_layout(%p, %d), con->type = %d\n",
         con, layout, con->type);
    ...
    if (con->type == CT_WORKSPACE) {
        if (con_num_children(con) == 0) {
            ...
            DEBUG("Attaching new split to ws\n");
            con_attach(new, con, false);

            tree_flatten(gConnRoot);
            con_force_split_parents_redraw(con);
            return;
        }
    }
...
static void con_on_remove_child(Con *con)
{
    ...
    if (con->type == CT_OUTPUT ||
        ...
        DEBUG("not handling, type = %d, name = %s\n", con->type, con->name);
        return;
    }
...
{
    DEBUG("Determining minimum size for con %p\n", con);
    ...
    if (con->layout == L_STACKED || con->layout == L_TABBED) {
        ...
        DEBUG("stacked/tabbed now, returning %d x %d + deco_rect = %d\n",
    ...
    ERROR("Unhandled case, type = %d, layout = %d, split = %d",
 ...
 * any of its descendants is gFocused, this function returns true if and only if
 * focusing con would mean that focus would still be visible on screen, i.e.,
 * the newly gFocused container would not be obscured by a fullscreen container.
 *
 ...
 * container can be gFocused under the fullscreen focus constraints, it can also
 * become a parent or sibling to the currently gFocused container.
 *
 */
bool con_fullscreen_permits_focusing(Con *con)
{
    ...
    g_assert(fs->fullscreen_mode != CF_NONE);
    ...
    /* Allow it only if the container to be gFocused is contained within the
...
void con_set_urgency(Con *con, bool urgent)
{
    ...
    if (con->urgent != old_urgent) {
        ERROR("Urgency flag changed to %d", con->urgent);
        ipc_send_window_event("urgent", con);
    }
}
 ...
 *
 */
GString* con_parse_title_format(Con *con)
{
    g_assert(con->title_format != NULL);
...
bool con_swap(Con *first, Con *second)
{
    ...
    if (ws1 == ws2 && ws1 == con_get_workspace(gFocused)) {
        /* Preserve focus in the current workspace. */
        restore_focus = gFocused;
    }
    else if (first == gFocused || con_has_parent(gFocused, first)) {
        restore_focus = second;
    }
    else if (second == gFocused || con_has_parent(gFocused, second)) {
        restore_focus = first;
    }
