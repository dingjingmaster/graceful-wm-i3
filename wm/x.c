...
/* Because 'gFocusedID' might be reset to force input focus, we separately keep
...
static con_state* state_for_frame(xcb_window_t window)
{
    ...
    ERROR("No state found for window 0x%08x\n", window);
    g_assert(false);
//    return NULL;
}

/*
 * Changes the atoms on the gRoot window and the windows themselves to properly
...
void x_con_init(Con *con)
{
    ...
    if (con->depth != gRootDepth) {
        /* We need to create a custom gColormap. */
        win_colormap = xcb_generate_id(gConn);
        xcb_create_colormap(gConn, XCB_COLORMAP_ALLOC_NONE, win_colormap, gRoot, visual);
        con->colormap = win_colormap;
    } else {
        /* Use the default gColormap. */
        win_colormap = gColormap;
    ...
    xcb_window_t frame_id = create_window(gConn, dims, con->depth, visual, XCB_WINDOW_CLASS_INPUT_OUTPUT, XCURSOR_CURSOR_POINTER, false, mask, values);
    draw_util_surface_init(gConn, &(con->frame), frame_id, get_visualtype_by_id(visual), dims.width, dims.height);
    ...
    DEBUG("adding new state for window id 0x%08x\n", state->id);
}
...
void x_reinit(Con *con)
{
    ...
    DEBUG("resetting state %p to initial\n", state);
    state->initial = true;
...
static void _x_con_kill(Con *con) {
    ...
    draw_util_surface_free(gConn, &(con->frame_buffer));
    xcb_free_pixmap(gConn, con->frame_buffer.id);
    ...
    /* Invalidate gFocusedID to correctly focus new windows with the same ID */
...
void x_window_kill(xcb_window_t window, kill_window_t kill_window) {
    ...
    if (!window_supports_protocol(window, A_WM_DELETE_WINDOW)) {
        if (kill_window == KILL_WINDOW) {
            DEBUG("Killing specific window 0x%08x\n", window);
            xcb_destroy_window(gConn, window);
        }
        else {
            DEBUG("Killing the X11 client which owns window 0x%08x\n", window);
    ...
    DEBUG("Sending WM_DELETE to the client\n");
...
void x_draw_decoration(Con *con)
{
    ...
    if (con_draw_decoration_into_frame(con)) {
        DEBUG("using con->frame_buffer (for con->name=%s) as dest_surface\n", con->name);
        dest_surface = &(con->frame_buffer);
    } else {
        DEBUG("sticking to parent->frame_buffer = %p\n", dest_surface);
    }
    DEBUG("dest_surface %p is %d x %d (id=0x%08x)\n", dest_surface, dest_surface->width, dest_surface->height, dest_surface->id);
    ...
    /* 4: paint the bar */
    DEBUG("con->deco_rect = (x=%d, y=%d, w=%d, h=%d) for con->name=%s\n",
...
static void set_hidden_state(Con *con)
{
    ...
    if (should_be_hidden) {
        ...
        xcb_add_property_atom(gConn, con->window->id, A__NET_WM_STATE, A__NET_WM_STATE_HIDDEN);
    }
    else {
        DEBUG("removing _NET_WM_STATE_HIDDEN for con = %p\n", con);
        xcb_remove_property_atom(gConn, con->window->id, A__NET_WM_STATE, A__NET_WM_STATE_HIDDEN);
    }
...
static void x_shape_frame(Con *con, xcb_shape_sk_t shape_kind)
{
    ...
    xcb_shape_combine(gConn, XCB_SHAPE_SO_SET, shape_kind, shape_kind,
                      con->frame.id,
    ...
    size_t rectangles_count = x_get_border_rectangles(con, rectangles);
    if (rectangles_count) {
        xcb_shape_rectangles(gConn, XCB_SHAPE_SO_UNION, shape_kind,
...
void x_push_node(Con *con) {
    ...
    if (state->name != NULL) {
        ...
        xcb_change_property(gConn, XCB_PROP_MODE_REPLACE, con->frame.id,
    ...
    if (state->need_reparent && con->window != NULL) {
        ...
        xcb_change_window_attributes(gConn, state->old_frame, XCB_CW_EVENT_MASK, values);
    ...
    /* The gRoot con and output cons will never require a pixmap. In particular for the
    ...
    if ((is_pixmap_needed && con->frame_buffer.id == XCB_NONE) || (!rect_equals(state->rect, rect) &&
        ...
        if (is_pixmap_needed && (has_rect_changed || con->frame_buffer.id == XCB_NONE)) {
            if (con->frame_buffer.id == XCB_NONE) {
                con->frame_buffer.id = xcb_generate_id(gConn);
            } else {
                draw_util_surface_free(gConn, &(con->frame_buffer));
                xcb_free_pixmap(gConn, con->frame_buffer.id);
            }
            ...
            xcb_create_pixmap(gConn, win_depth, con->frame_buffer.id, con->frame.id, width, height);
            draw_util_surface_init(gConn, &(con->frame_buffer), con->frame_buffer.id,
            ...
            xcb_change_gc(gConn, con->frame_buffer.gc, XCB_GC_GRAPHICS_EXPOSURES, (uint32_t[]){0});
         ...
         * fast as possible) */
        xcb_flush(gConn);
        xcb_set_window_rect(gConn, con->frame.id, rect);
        if (con->frame_buffer.id != XCB_NONE) {
            draw_util_copy_surface(&(con->frame_buffer), &(con->frame), 0, 0, 0, 0, con->rect.width, con->rect.height);
        }
        xcb_flush(gConn);
    ...
    if (con->window != NULL &&
        ...
        xcb_set_window_rect(gConn, con->window->id, con->window_rect);
    ...
    if ((state->mapped != con->mapped || (con->window != NULL && !state->child_mapped)) &&
        ...
        if (con->window != NULL) {
            ...
            xcb_change_property(gConn, XCB_PROP_MODE_REPLACE, con->window->id,
...
void x_push_changes(Con *con) {
    ...
    if (gFocusedID != to_focus) {
        if (!focused->mapped) {
            DEBUG("Not updating focus (to %p / %s), focused window is not mapped.\n", focused, focused->name);
            /* Invalidate gFocusedID to correctly focus new windows with the same ID */
            gFocusedID = XCB_NONE;
        } else {
            if (focused->window != NULL &&
                ...
                DEBUG("Updating focus by sending WM_TAKE_FOCUS to window 0x%08x (focused: %p / %s)\n",
                     to_focus, focused, focused->name);
                send_take_focus(to_focus, last_timestamp);

                change_ewmh_focus((con_has_managed_window(focused) ? focused->window->id : XCB_WINDOW_NONE), gLastFocused);

                if (to_focus != gLastFocused && is_con_attached(focused))
                ...
                if (focused->window != NULL) {
                    ...
                    xcb_change_window_attributes(gConn, focused->window->id, XCB_CW_EVENT_MASK, values);
                }
                xcb_set_input_focus(gConn, XCB_INPUT_FOCUS_POINTER_ROOT, to_focus, last_timestamp);
                if (focused->window != NULL) {
                    values[0] = CHILD_EVENT_MASK;
                    xcb_change_window_attributes(gConn, focused->window->id, XCB_CW_EVENT_MASK, values);
                }
                ...
                if (to_focus != XCB_NONE && to_focus != gLastFocused && focused->window != NULL && is_con_attached(focused))
                    ipc_send_window_event("focus", focused);
            }
    ...
    if (gFocusedID == XCB_NONE) {
         ...
         * gRoot window in order to avoid an X11 fallback mechanism causing a ghosting effect (see #1378). */
        DEBUG("Still no window focused, better set focus to the EWMH support window (%d)\n", ewmh_window);
        xcb_set_input_focus(gConn, XCB_INPUT_FOCUS_POINTER_ROOT, ewmh_window, last_timestamp);
        change_ewmh_focus(XCB_WINDOW_NONE, gLastFocused);

        gFocusedID = ewmh_window;
        gLastFocused = XCB_NONE;
    }
 ...
 * i3 windows in xwininfo -gRoot -all.
 *
 */
...
void update_shmlog_atom(void) {
    if (*shmlogname == '\0') {
        xcb_delete_property(gConn, gRoot, A_I3_SHMLOG_PATH);
    } else {
        xcb_change_property(gConn, XCB_PROP_MODE_REPLACE, gRoot,
...
void x_set_i3_atoms(void) {
    pid_t pid = getpid();
    xcb_change_property(gConn, XCB_PROP_MODE_REPLACE, gRoot, A_I3_SOCKET_PATH, A_UTF8_STRING, 8,
                        (current_socketpath == NULL ? 0 : strlen(current_socketpath)),
                        current_socketpath);
    xcb_change_property(gConn, XCB_PROP_MODE_REPLACE, gRoot, A_I3_PID, XCB_ATOM_CARDINAL, 32, 1, &pid);
    xcb_change_property(gConn, XCB_PROP_MODE_REPLACE, gRoot, A_I3_CONFIG_PATH, A_UTF8_STRING, 8,
                        strlen(current_configpath), current_configpath);
    xcb_change_property(gConn, XCB_PROP_MODE_REPLACE, gRoot, A_I3_LOG_STREAM_SOCKET_PATH, A_UTF8_STRING, 8,
                        strlen(current_log_stream_socket_path), current_log_stream_socket_path);
    update_shmlog_atom();
}

/*
 * Set gWarpTo coordinates.  This will trigger on the next call to
 * x_push_changes().
 *
 */
