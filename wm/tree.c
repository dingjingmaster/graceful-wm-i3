...
{
    Con *__i3 = con_new(gConnRoot, NULL);
    FREE(__i3->name);
    __i3->name = sstrdup("__i3");
    __i3->type = CT_OUTPUT;
    __i3->layout = L_OUTPUT;
    con_fix_percent(gConnRoot);
...
bool tree_restore(const char *path, xcb_get_geometry_reply_t *geometry)
{
    ...
    if (!path_exists(globbed)) {
        DEBUG("%s does not exist, not restoring tree", globbed);
        goto out;
    }
    ...
    DEBUG("appended tree, using new root\n");
    gConnRoot = TAILQ_FIRST(&(gConnRoot->nodes_head));
    if (!gConnRoot) {
        /* tree_append_json failed. Continuing here would segfault. */
        goto out;
    }
    DEBUG("new root = %p\n", gConnRoot);
    Con *out = TAILQ_FIRST(&(gConnRoot->nodes_head));
    DEBUG("out = %p\n", out);
    ...
    if (strcmp(out->name, "__i3") != 0) {
        DEBUG("Adding pseudo-output __i3 during inplace restart\n");
        Con *__i3 = _create___i3();
        /* Ensure that it is the first output, other places in the code make
         * that assumption. */
        TAILQ_REMOVE(&(gConnRoot->nodes_head), __i3, nodes);
        TAILQ_INSERT_HEAD(&(gConnRoot->nodes_head), __i3, nodes);
    }

    restore_open_placeholder_windows(gConnRoot);
    result = true;
...
void tree_init(xcb_get_geometry_reply_t *geometry)
{
    ...
    FREE(gConnRoot->name);
    gConnRoot->name = "root";
    gConnRoot->type = CT_ROOT;
    gConnRoot->layout = L_SPLITH;
    gConnRoot->rect = (Rect) {
        geometry->x,
        geometry->y,
        geometry->width,
...
Con *tree_open_con(Con *con, i3Window *window) {
    if (con == NULL) {
        ...
        /* If the currently gFocused container is a floating container, we
         * attach the new container to the currently gFocused spot in its
         * workspace. */
...
bool tree_close_internal(Con *con, kill_window_t kill_window, bool dont_kill_parent) {
    ...
    if (abort_kill) {
        DEBUG("One of the children could not be killed immediately (WM_DELETE sent), aborting.\n");
        return false;
    }
    ...
    Con *next = (con == gFocused) ? con_next_focused(con) : NULL;
    DEBUG("next = %p, gFocused = %p\n", next, gFocused);
    ...
    if (con->urgency_timer != NULL) {
        DEBUG("Removing urgency timer of con %p\n", con);
    ...
    if (ws == con) {
        DEBUG("Closing workspace container %s, updating EWMH atoms\n", ws->name);
    ...
    if (next) {
        ...
        DEBUG("not changing focus, the container was not gFocused before\n");
    }
...
void tree_split(Con *con, orientation_t orientation) {
    if (con_is_floating(con)) {
        DEBUG("Floating containers can't be split.\n");
        return;
    }
    ...
    if (con->type == CT_WORKSPACE) {
        if (con_num_children(con) < 2) {
            if (con_num_children(con) == 0) {
                DEBUG("Changing workspace_layout to L_DEFAULT\n");
                con->workspace_layout = L_DEFAULT;
            }
            DEBUG("Changing orientation of workspace\n");
    ...
    if (con_num_children(parent) == 1 &&
        ...
        DEBUG("Just changing orientation of existing container\n");
        return;
    }

    DEBUG("Splitting in orientation %d\n", orientation);

    /* 2: replace it with a new Con */
    Con *new = con_new(NULL, NULL);
    TAILQ_REPLACE(&(parent->nodes_head), con, new, nodes);
    TAILQ_REPLACE(&(parent->focus_head), con, new, gFocused);
    new->parent = parent;
...
bool level_up(void) {
    ...
    if (gFocused->parent->type == CT_FLOATING_CON) {
        con_activate(gFocused->parent->parent);
        return true;
    }
...
bool level_down(void) {
    ...
    Con *next = TAILQ_FIRST(&(gFocused->focus_head));
    if (next == TAILQ_END(&(gFocused->focus_head))) {
        DEBUG("cannot go down\n");
        return false;
    } else if (next->type == CT_FLOATING_CON) {
        /* Floating cons shouldn't be directly gFocused; try immediately
         * going to the grandchild of the gFocused con. */
...
void tree_render(void) {
    ...
    mark_unmapped(gConnRoot);
    gConnRoot->mapped = true;

    render_con(gConnRoot);

    x_push_changes(gConnRoot);
    DEBUG("-- END RENDERING --\n");
}

static Con *get_tree_next_workspace(Con *con, direction_t direction) {
    if (con_get_fullscreen_con(con, CF_GLOBAL)) {
        DEBUG("Cannot change workspace while in global fullscreen mode.\n");
        return NULL;
    }
...
void tree_flatten(Con *con) {
    ...
    DEBUG("Checking if I can flatten con = %p / %s\n", con, con->name);
    ...
    DEBUG("Alright, I have to flatten this situation now. Stay calm.\n");
    /* 1: save focus */
    ...
    while (!TAILQ_EMPTY(&(child->nodes_head))) {
        ...
        DEBUG("attaching to focus list\n");
        TAILQ_INSERT_TAIL(&(parent->focus_head), current, gFocused);
        current->percent = con->percent;
    }
    DEBUG("re-attached all\n");

    /* 3: restore focus, if con was gFocused */
    if (focus_next != NULL &&
        TAILQ_FIRST(&(parent->focus_head)) == con) {
        DEBUG("restoring focus to focus_next=%p\n", focus_next);
        TAILQ_REMOVE(&(parent->focus_head), focus_next, gFocused);
        TAILQ_INSERT_HEAD(&(parent->focus_head), focus_next, gFocused);
        DEBUG("restored focus.\n");
    }

    /* 4: close the redundant cons */
    DEBUG("closing redundant cons\n");
