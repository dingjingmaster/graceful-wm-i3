...
static TAILQ_HEAD(state_head, placeholder_state) state_head = TAILQ_HEAD_INITIALIZER(state_head);
...
static void restore_xcb_prepare_cb (EV_P_ ev_prepare *w, int revents)
{
    ...
    if (xcb_connection_has_error(restore_conn)) {
        LOG_DEBUG("restore X11 connection has an error, reconnecting");
        restore_connect();
        return;
    }
    ...
    while ((event = xcb_poll_for_event(restore_conn)) != NULL) {
        if (event->response_type == 0) {
            ...
            LOG_DEBUG("X11 Error received (probably harmless)! sequence 0x%x, error_code = %d", error->sequence, error->error_code);
            free(event);
            continue;
        }
...
void restore_connect(void)
{
    if (restore_conn != NULL) {
        ...
        ev_io_stop(gMainLoop, xcb_watcher);
        ev_prepare_stop(gMainLoop, xcb_prepare);
...
#define APPEND_REGEX(re_name)                                                                                                                        \
    do {                                                                                                                                             \
        ...
        if (serialized == NULL) {
            LOG_DEBUG("This swallows specification is not serializable?!\n");
            continue;
        }

        sasprintf(&serialized, "%s]", serialized);
        LOG_DEBUG("con %p (placeholder 0x%08x) line %d: %s\n", state->con, state->window, n, serialized);
...
static void open_placeholder_window(Con *con)
{
    if (con_is_leaf(con) &&
        ...
        LOG_DEBUG("Created placeholder window 0x%08x for leaf container %p / %s\n",
        ...
        draw_util_surface_init(restore_conn, &(state->surface), placeholder, get_visual_type(gRootScreen), state->rect.width, state->rect.height);
        ...
        Match *tempID = smalloc(sizeof(Match));
        match_init(tempID);
        tempID->dock = M_DONTCHECK;
        tempID->id = placeholder;
        TAILQ_INSERT_HEAD(&(con->swallow_head), tempID, matches);
    }
...
bool restore_kill_placeholder(xcb_window_t placeholder)
{
    ...
    TAILQ_FOREACH (state, &state_head, state) {
        ...
        TAILQ_REMOVE(&state_head, state, state);
        free(state);
        LOG_DEBUG("placeholder window 0x%08x destroyed.\n", placeholder);
        return true;
    }

    LOG_DEBUG("0x%08x is not a placeholder window, ignoring.\n", placeholder);
    return false;
}
...
static void expose_event(xcb_expose_event_t *event) {
    ...
    TAILQ_FOREACH (state, &state_head, state) {
        ...
        LOG_DEBUG("refreshing window 0x%08x contents (con %p)\n", state->window, state->con);
    ...
    LOG_ERROR("Received ExposeEvent for unknown window 0x%08x\n", event->window);
}
...
static void configure_notify(xcb_configure_notify_event_t *event)
{
    ...
    TAILQ_FOREACH (state, &state_head, state) {
        ...
        LOG_DEBUG("ConfigureNotify: window 0x%08x has now width=%d, height=%d (con %p)",
    ...
    LOG_ERROR("Received ConfigureNotify for unknown window 0x%08x\n", event->window);
}
...
static void restore_handle_event(int type, xcb_generic_event_t *event) {
    switch (type) {
        ...
        case XCB_CONFIGURE_NOTIFY:
            ...
            break;
        default:
            LOG_DEBUG("Received unhandled X11 event of type %d\n", type);
            break;
    }
}
