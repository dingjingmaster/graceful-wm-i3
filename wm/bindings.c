                           ...
                           bool pango_markup)
{
    ...
    DEBUG("Binding %p bindtype %s, modifiers %s, input code %s, release %s\n", new_binding, bindtype, modifiers, input_code, release);
    ...
    if (strcmp(bindtype, "bindsym") == 0) {
        ...
        if (!parse_long(input_code, &keycode, 10)) {
            ERROR("Could not parse \"%s\" as an input code, ignoring this binding.\n", input_code);
            FREE(new_binding);
            return NULL;
        }
    ...
    if ((new_binding->event_state_mask >> 16) & I3_XKB_GROUP_MASK_4)
        group_bits_set++;
    if (group_bits_set > 1)
        ERROR("Keybinding has more than one Group specified, but your X server is always in precisely one group. The keybinding can never trigger.");
...
static bool binding_in_current_group(const Binding *bind)
{
    ...
    switch (xkb_current_group) {
        ...
        default:
            ERROR("BUG: xkb_current_group (= %d) outside of [XCB_XKB_GROUP_1..XCB_XKB_GROUP_4]\n", xkb_current_group);
            return false;
    }
}
...
#define GRAB_KEY(modifier)                                                                       \
    ...
    DEBUG("Binding %p Grabbing keycode %d with event state mask 0x%x (mods 0x%x)",
...
void regrab_all_buttons(xcb_connection_t *conn)
{
    ...
    Con *con;
    TAILQ_FOREACH (con, &gAllCons, all_cons) {
        if (con->window == NULL)
            continue;
...
static Binding *get_binding(i3_event_state_mask_t state_filtered, bool is_release, uint16_t input_code, input_type_t input_type)
{
    ...
    TAILQ_FOREACH (bind, gBindings, bindings) {
        ...
        if (!groups_match) {
            DEBUG("skipping binding %p because XKB groups do not match", bind);
            continue;
        }
        ...
        if (input_type == B_KEYBOARD && bind->symbol != NULL) {
            ...
            TAILQ_FOREACH (binding_keycode, &(bind->keycodes_head), keycodes) {
                ...
                DEBUG("binding_keycode->modifiers = %d, modifiers_mask = %d, modifiers_state = %d, mods_match = %s",
        ...
        if (bind->release == B_UPON_KEYRELEASE && !is_release) {
            ...
            DEBUG("marked bind %p as B_UPON_KEYRELEASE_IGNORE_MODS\n", bind);
            if (result) {
                break;
            }
            continue;
...
Binding *get_binding_from_xcb_event(xcb_generic_event_t *event)
{
    ...
    DEBUG("(removed capslock, state = 0x%x)\n", state_filtered);
    ...
    DEBUG("(transformed keyboard group, state = 0x%x)\n", state_filtered);
...
static void add_keycode_if_matches(struct xkb_keymap *keymap, xkb_keycode_t key, void *data)
{
    ...
    if ((bind->event_state_mask & xcb_numlock_mask) != xcb_numlock_mask) {
        ...
        if (sym_numlock == resolving->keysym) {
            ...
            DEBUG("Skipping automatic numlock fallback, key %d resolves to 0x%x with numlock", key, sym_numlock);
        }
    }
}
...
void check_for_duplicate_bindings(struct context *context)
{
    ...
    TAILQ_FOREACH (current, gBindings, bindings) {
        TAILQ_FOREACH (bind, gBindings, bindings) {
...
static int fill_rmlvo_from_root(struct xkb_rule_names *xkb_names)
{
    ...
    prop_cookie = xcb_get_property_unchecked(conn, false, gRoot, atom_reply->atom,
