...
/* The default gVisualType to use if none is specified when creating the surface. Must be defined globally. */
...
static bool surface_initialized(surface_t *surface)
{
    if (surface->id == XCB_NONE) {
        LOG_ERROR("Surface %p is not initialized, skipping drawing.", surface);
        return false;
    }
    return true;
}
                            ...
                            int height)
{
    ...
    if (error != NULL) {
        LOG_ERROR("Could not create graphical context. Error code: %d. Please report this bug.\n", error->error_code);
        free(error);
    }
...
void draw_util_surface_free(xcb_connection_t *conn, surface_t *surface)
{
    ...
    if (status != CAIRO_STATUS_SUCCESS) {
        LOG_DEBUG("Found cairo context in an error status while freeing, error %d is %s",
...
color_t draw_util_hex_to_color(const char *color)
{
    if (strlen(color) < 6 || color[0] != '#') {
        LOG_ERROR("Could not parse color: %s\n", color);
